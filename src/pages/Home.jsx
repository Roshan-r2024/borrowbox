...
function Home() {
  ...
  return (
    <div className="home-page">
      <header className="navbar">
        <div className="nav-container">
          <div className="brand" onClick={() => navigate("/home")}>
            <div className="brand-logo">◇</div><div className="brand-text"><h2>Borrow Box</h2><span>Campus sharing</span></div>
          </div>
      ...
      <main className="home-main">
        <section className="hero-section">
          <div className="hero-left">
            ...
            <h1>Borrow what you need.<br/><span>Lend what you have.</span></h1>
            <p className="hero-description">A simple way for students to share, borrow and lend useful items within the campus community.</p>
          </div>
          <div className="hero-search-area">
            <div className="search-wrapper"><div className="search-icon">⌕</div><input placeholder="Search books, electronics, notes..." onKeyDown={e => { if (e.key === "Enter") navigate("/browse"); }}/></div>
            <div className="home-category-strip">
              <div className="home-category-heading"><span>QUICK CATEGORIES</span><button onClick={() => navigate("/browse")}>View all →</button></div>
              <div className="home-category-list">
                {categories.map(c => <button key={c.name} className="home-category-pill" onClick={() => browseCategory(c.name)}><span>{c.icon}</span>{c.name}</button>)}
              </div>
            </div>
          </div>
        </section>
      ...
      <footer className="home-footer"><div className="footer-content"><div><strong>Borrow Box</strong><span>Campus sharing made simple.</span></div><button onClick={logout}>Logout</button></div></footer>
    </div>
  );
}
export default Home;
