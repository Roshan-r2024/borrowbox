...
function ItemDetails() {
  ...
  const handleBorrowRequest = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(
        `Borrow request sent successfully. Priority: ${urgency}. The owner will review your request.`
      );
      setSuccess(true);
      setStartDate("");
      setEndDate("");
    ...
    <div className="item-details-page">
      <header className="details-navbar">
        <div className="details-nav-container">
          <div className="details-brand" onClick={() => navigate("/home")}>
          ...
          <button onClick={() => navigate("/browse")} className="back-browse">
            ← Browse Items
          </button>
        </div>
      </header>
      ...
      <main className="item-details-container">
        <div className="details-layout">
          <section className="details-image-section">
            <div className={`details-image ${item.displayStyle || "square"}`}>
              {item.imageUrl ? (
          ...
          <section className="details-info">
            <div className="details-topline">
              ...
              <span className="details-type">RENT</span>
            </div>
            ...
            <div className="details-owner">
              ...
              <div>
                <span>Owned by</span>
                <strong>{item.owner || "Student"}</strong>
              </div>
            </div>

            <div className="borrow-request-box">
              <div className="borrow-request-heading">
                <h2>Borrow this item</h2>
                <p>Select the dates and tell the owner what you need.</p>
              </div>
              ...
              {!success && (
                <form onSubmit={handleBorrowRequest}>
                  ...
                  <div className="urgency-field">
                    <label>Choose an option you need</label>
                    <div className="urgency-options">
                      <button
                        type="button"
                        className={urgency === "Normal" ? "urgency-option selected" : "urgency-option"}
                        onClick={() => setUrgency("Normal")}
                      >
                        <strong>Normal</strong>
                        <span>Regular request</span>
                      </button>
                      <button
                        type="button"
                        className={urgency === "Urgent" ? "urgency-option selected urgent" : "urgency-option"}
                        onClick={() => setUrgency("Urgent")}
                      >
                        <strong>Urgent</strong>
                        <span>Need it quickly</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <button type="submit" className="send-request-btn" disabled={sending}>
                    {sending ? "Sending Request..." : "Borrow Item →"}
                  </button>

                  <p className="request-note">
                    Your request will be sent to the item owner for approval.
                  </p>
                </form>
              )}

              {success && (
                <button className="back-after-success" onClick={() => navigate("/browse")}>
                  ← Continue Browsing
                </button>
              )}
            </div>
