...
function ListItem() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ message: "Network error while listing item. Please try again.", type: "error" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="list-container">
        ...
        <form className="list-form" onSubmit={handleSubmit}>
          <section className="form-section"><div className="section-heading"><div><h2>Item Photo</h2><p className="section-desc">Upload a clear photo and choose the best frame.</p></div><span className="required-text">Required</span></div>
            <div className="frame-section"><label>Image Display</label><div className="frame-options">{[["square","Square","1:1","General"],["portrait","Portrait","4:5","Books / Mobile"],["landscape","Landscape","16:10","Laptop / Monitor"]].map(([value,title,ratio,desc]) => <button key={value} type="button" className={displayStyle === value ? "frame-option selected" : "frame-option"} onClick={() => setDisplayStyle(value)}><strong>{title}</strong><span>{ratio}</span><small>{desc}</small></button>)}</div></div>
          ...
          <section className="form-section"><div className="section-heading"><div><h2>Item Details</h2><p className="section-desc">Provide accurate information about your item.</p></div></div>
            <div className="form-grid">
              ...
              <div className="input-group"><label>Listing type</label><div className="listing-type-options"><label><input type="radio" name="listingType" value="rent" checked={formData.listingType === "rent"} onChange={handleChange} disabled={loading} /> Rent</label><label><input type="radio" name="listingType" value="sale" checked={formData.listingType === "sale"} onChange={handleChange} disabled={loading} /> Sale</label></div></div>
              <div className="input-group"><label htmlFor="price">{formData.listingType === "sale" ? "Sale price" : "Price per day"}</label><div className="price-input"><span>₹</span><input id="price" name="price" type="number" min="0" placeholder={formData.listingType === "sale" ? "500" : "50"} value={formData.price} onChange={handleChange} disabled={loading} /></div></div>
              <div className="input-group full"><label htmlFor="description">Description</label><textarea id="description" name="description" rows="5" placeholder="Describe your item, its condition and anything the student should know..." value={formData.description} onChange={handleChange} disabled={loading} /></div>
