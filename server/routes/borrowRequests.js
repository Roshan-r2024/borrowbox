...
router.post("/", async (req, res) => {
  try {
    ...
    const request = await BorrowRequest.create({
      ...
      borrower: borrower.trim(), borrowerEmail: borrowerEmail.trim().toLowerCase(), borrowerPhone: String(borrowerPhone || "").trim(),
      owner: item.owner, ownerEmail: item.ownerEmail || "", startDate: start, endDate: end, message: message?.trim() || "",
      landmark: String(landmark).trim(), location: { latitude: lat, longitude: lng },
...
router.put("/:id/:action", async (req, res) => {
  try {
    ...
    if (action === "approve") {
      ...
      await item.save();
      return res.status(200).json({ message: request.requestType === "Purchase" ? "Purchase order approved." : "Borrow request approved.", request });
    }
