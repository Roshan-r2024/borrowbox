...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (!cleanEmail.endsWith("@vitstudent.ac.in")) {
      return res.status(400).json({ message: "Please use your VIT student email." });
    }
...
// UPDATE PROFILE — supports nickname, phone and profile picture
router.put("/profile", (req, res) => {
  upload.single("profilePicture")(req, res, async (uploadError) => {
    if (uploadError) {
      return res.status(400).json({ message: uploadError.message || "Profile picture upload failed." });
    }
